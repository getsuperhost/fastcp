...
class DatabaseSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        ...
        if value in DISALLOWED_NAMES:
            raise serializers.ValidationError(f"{value} is not allowed to be used as a database name.")
        return value

    def validate_username(self, value):
        """Ensure that it doesn't use a preserved username."""
        if value in DISALLOWED_NAMES:
            raise serializers.ValidationError(f"{value} is not allowed to be used as a username.")
        return value
