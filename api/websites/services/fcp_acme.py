...
class FastcpAcme(object):
    ...
    def _register_account(self, email: str = None):
        ...
        return self.client.new_account(messages.NewRegistration.from_data(email=None, terms_of_service_agreed=True))
    ...
    def request_ssl(self, domains: list, priv_key: str = None, chall_type="http") -> dict:
        ...
        for chall in self._select_chall(self.acme_order, chall_type=chall_type):
            ...
            response, validation = chall.chall.response_and_validation(account_key=self.acc_key)
            self.response = response

            # Get challenge path
            challange_path = os.path.join(challenges.HTTP01.URI_ROOT_PATH, chall.chall.encode("token"))

            # Challenge token
            challenge_token = validation.encode()

            token_paths.append({"path": challange_path, "token": challenge_token})
    ...
    def get_ssl(self):
        ...
        try:
            ...
            return {"full_chain": order_result.fullchain_pem, "priv_key": self.priv_key}
        except Exception:
            return False
